...
// Katak ichidagi elementni generatsiya qilish (Refreshsiz yangilash uchun kerak)
...
<head>
    ...
    <style>
        ...
        .cell { border: 1px solid var(--border); width: 65px; height: 65px; display: flex; align-items: center; justify-content: center; position: relative; border-radius: 4px; background: rgba(0,0,0,0.1); }
        ...
        #theme-toggle { position: fixed; top: 20px; right: 20px; padding: 8px 15px; border-radius: 20px; border: 1px solid var(--border); background: var(--card); color: var(--text); cursor: pointer; }
        .found { border: 2px solid #d29922 !important; box-shadow: 0 0 10px #d29922; }
    </style>
</head>
<body id="b">
    <button id="theme-toggle" onclick="toggleTheme()">Rejimni o'zgartirish</button>
    <input type="text" id="search" placeholder="Qidiruv..." style="margin-bottom:20px; padding:8px; border-radius:6px; border:1px solid var(--border); background:var(--card); color:var(--text);" oninput="doSearch()">
    ${html}

    <div id="modal" style="display:none; position:fixed; top:50%; left:50%; transform:translate(-50%,-50%); background:var(--card); padding:20px; border:1px solid var(--border); border-radius:8px; z-index:100;">
        <input type="text" id="newCode" placeholder="Kod">
        <button onclick="saveItem()">Saqlash</button>
    ...
    <script>
        ...
        function doSearch() {
            ...
            document.querySelectorAll('.inventory-item').forEach(i => {
                if(i.dataset.code.includes(s)) i.closest('.cell').classList.add('found');
            });
        }
    </script>
</body>
...
app.listen(3000, '0.0.0.0', () => console.log('Tayyor: port 3000'));
