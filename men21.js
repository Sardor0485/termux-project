...
// Katak ichini yaratish (Ajax uchun)
...
<head>
    ...
    <style>
        ...
        .cell { border: 1px solid var(--border); width: 65px; height: 65px; display: flex; align-items: center; justify-content: center; position: relative; border-radius: 4px; transition: 0.3s; }
        
        /* TOPILGAN KATAK UCHUN RANG (Shadow emas) */
        .found { background: #d29922 !important; border-color: #e3b341 !important; }
        .found .item-code, .found .item-qty { color: #000 !important; }
        ...
        .btn-m, .btn-p { border: none; border-radius: 4px; color: white; cursor: pointer; padding: 2px 8px; font-weight: bold; }
        .btn-m { background: #f85149; } .btn-p { background: #3fb950; }
        .btn-add { color: var(--border); cursor: pointer; font-size: 24px; }
        
        #theme-toggle { position: fixed; top: 20px; right: 20px; padding: 8px 15px; border-radius: 20px; border: 1px solid var(--border); background: var(--card); color: var(--text); cursor: pointer; z-index: 1000; }
        #search { position: fixed; top: 20px; left: 50%; transform: translateX(-50%); width: 250px; padding: 10px; border-radius: 20px; border: 1px solid var(--border); background: var(--card); color: var(--text); z-index: 1000; outline: none; }
        #search:focus { border-color: var(--accent); }
    </style>
</head>
<body id="b">
    <button id="theme-toggle" onclick="toggleTheme()">🌓 Mode</button>
    <input type="text" id="search" placeholder="Kodni yozing..." oninput="doSearch()">
    
    <div style="margin-top: 60px;"> ${html}
    </div>

    <div id="modal" style="display:none; position:fixed; top:50%; left:50%; transform:translate(-50%,-50%); background:var(--card); padding:20px; border:1px solid var(--border); border-radius:8px; z-index:2000; box-shadow: 0 0 20px rgba(0,0,0,0.5);">
        <input type="text" id="newCode" placeholder="Kod" style="padding:8px; margin-bottom:10px; width:100%;">
        <button onclick="saveItem()" style="width:100%; padding:8px; background:var(--accent); border:none; color:white; border-radius:4px; cursor:pointer;">Saqlash</button>
        <button onclick="closeModal()" style="width:100%; margin-top:5px; background:transparent; color:var(--text); border:none; cursor:pointer;">Yopish</button>
    </div>
    ...
    <script>
        ...
        function doSearch() {
            const s = document.getElementById('search').value.toUpperCase().trim();
            ...
            document.querySelectorAll('.inventory-item').forEach(i => {
                if(i.dataset.code.includes(s)) {
                    ...
                    if(!firstFound) firstFound = cell;
                }
            });

            if(firstFound) {
                // Avtomatik silliq skroll qilish
