...
<head>
    <style>
        ...
        /* SCROLL BARNI YO'QOTISH */
        .racks-container { 
            display: flex; gap: 20px; padding: 10px 20px 40px; 
            overflow-x: auto; -ms-overflow-style: none; scrollbar-width: none; 
        }
        .racks-container::-webkit-scrollbar { display: none; }

        .header { position: fixed; top: 0; width: 100%; height: 55px; background: var(--card); border-bottom: 1px solid var(--brd); display: flex; align-items: center; padding: 0 20px; z-index: 1000; box-sizing: border-box; gap: 10px; }
        input#srch { flex: 1; background: var(--bg); border: 1px solid var(--brd); color: var(--txt); padding: 8px; border-radius: 6px; outline: none; }
        
        .rack-box { background: var(--card); border: 1px solid var(--brd); border-radius: 12px; padding: 12px; min-width: 320px; }
        .floor-grid { display: flex; gap: 6px; margin-bottom: 6px; }

        /* KATAKLAR */
        .cell { flex: 1; height: 90px; border: 1px solid var(--brd); border-radius: 10px; background: var(--bg); position: relative; display: flex; align-items: center; justify-content: center; overflow: hidden; transition: 0.3s; }
        .cell.empty { border: 2px dashed var(--brd); opacity: 0.5; font-size: 24px; cursor: pointer; }
        
        /* KOD: ORIGINAL RANG VA 2 BAROBAR KATTA */
        .code-label { position: absolute; top: 6px; font-size: 18px; font-weight: 800; color: #8b949e; z-index: 5; pointer-events: none; }
        .qty-val { font-size: 32px; font-weight: 900; z-index: 5; }
        ...
        .glint { position: absolute; width: 200%; height: 100%; background: linear-gradient(110deg, transparent, rgba(255,255,255,0.08), transparent); transform: translateX(-150%); }
        .ripple { position: absolute; top: 50%; left: 50%; width: 2px; height: 2px; background: rgba(255,255,255,0.3); border-radius: 50%; transform: translate(-50%,-50%) scale(0); opacity: 0; }
        .cell:hover .glint { transform: translateX(100%); transition: 0.8s; }
        .cell:hover .ripple { animation: rip 0.8s ease-out forwards; }
        @keyframes rip { 0% { scale: 0; opacity: 0.6; } 100% { scale: 30; opacity: 0; } }

        /* KOMETA 45 GRADUS */
        .comet-star { position: absolute; width: 100px; height: 1px; background: linear-gradient(90deg, #fff, transparent); transform: rotate(-45deg); top: -100px; left: -100px; opacity: 0; z-index: 6; }
        .cell:hover .comet-star { animation: fly 0.6s ease-out; }
        @keyframes fly { 0% { top: -30px; left: -30px; opacity: 1; } 100% { top: 120px; left: 120px; opacity: 0; } }

        /* QONG'IROQ */
        .bell-shake { animation: shk 0.8s infinite; animation-delay: var(--d); transform-origin: top center; }
        @keyframes shk { 0%, 100% { transform: rotate(0); } 25% { transform: rotate(4deg); } 75% { transform: rotate(-4deg); } }
        ...
        .btns button { flex: 1; border: 0; background: var(--blue); color: #fff; cursor: pointer; height: 25px; font-weight: bold; }
...
<body>
    ...
    <div id="modal">
        <div class="m-box">
            <h3>Yangi Tavar</h3>
            <input id="mCode" placeholder="Kod" style="width:100%; margin-bottom:10px; padding:10px; box-sizing:border-box;">
            <input id="mQty" type="number" value="1" style="width:100%; margin-bottom:10px; padding:10px; box-sizing:border-box;">
            <button onclick="save()" style="width:100%; padding:10px; background:var(--green); border:0; color:#fff; border-radius:5px; cursor:pointer;">SAQLASH</button>
            <button onclick="document.getElementById('modal').style.display='none'" style="width:100%; margin-top:5px; background:none; color:var(--red); border:0; cursor:pointer;">Bekor qilish</button>
        </div>
    </div>
    ...
    <script>
        ...
        function handleDrag(e, el) { e.dataTransfer.setData("text", el.id); el.classList.add('dragging'); }
