...
<head>
    ...
    <style>
        ...
        #sInp { flex: 1; padding: 12px; border-radius: 8px; background: var(--input); color: var(--text); border: 1px solid var(--border); outline: none; transition: 0.3s; }
        #sInp:focus { border-color: var(--gold); box-shadow: 0 0 10px rgba(255, 215, 0, 0.3); }

        .racks-container { display: flex; gap: 25px; padding: 20px; overflow-x: auto; align-items: flex-start; }
        .rack { background: var(--card); border-radius: 12px; padding: 15px; min-width: 320px; border: 1px solid var(--border); flex-shrink: 0; transition: 0.3s; }
        ...
        .item-content {
            ...
            background: var(--input); position: relative; overflow: hidden; transition: background 0.5s, transform 0.3s, border-color 0.3s;
        }

        /* --- QIDIRUVDA TOPILGAN KATAKLAR --- */
        ...
        .search-highlight .item-code { color: #000 !important; font-weight: 800; }
        .search-highlight .item-qty { color: #000 !important; font-weight: 900; animation: none !important; text-shadow: none !important; }

        /* --- 45 GRADUS KOMETA ANIMATSIYASI --- */
        .comet {
        ...
        .comet::after {
            ...
            transform: rotate(-135deg); transform-origin: left;
        }

        .inventory-item:hover .comet {
            animation: diagonal-comet 0.7s ease-out forwards;
        }
        @keyframes diagonal-comet {
            0% { transform: translate(-20px, -20px); opacity: 0; }
            30% { opacity: 1; }
            100% { transform: translate(140px, 110px); opacity: 0; }
        }

        /* Border effekti va boshqalar */
        ...
        .qty-warning { color: var(--orange-mitter); text-shadow: 0 0 5px rgba(255, 140, 0, 0.3); }
        .qty-danger { color: var(--red); text-shadow: 0 0 8px rgba(255, 68, 68, 0.5); }
...
<body onclick="closeAllActive(event)">
    ...
    <script>
        ...
        // QIDIRUV MANTIQI (SARIQ RANG VA SCROLL)
        function handleSearch() {
            ...
            items.forEach(i => {
                i.classList.remove('search-highlight');
                i.style.opacity = "1";
            });
            ...
            items.forEach(i => {
                if(i.dataset.code && i.dataset.code.includes(val)) {
                    i.classList.add('search-highlight'); // Sariq rangga bo'yash
                    if(!firstFound) firstFound = i;
                } else {
                    // Topilmaganlarini xiralashtirish (ixtiyoriy, chiroyli ko'rinadi)
                    i.style.opacity = "0.1";
                }
            });
            
            // Birinchi topilganiga scroll qilish
            if(firstFound) {
...
// Portni band bo'lishidan xavfsiz start
const server = app.listen(3000, () => console.log('Server running on port 3000'));
server.on('error', (e) => {
    if (e.code === 'EADDRINUSE') {
        console.log('PORT 3000 BAND! Iltimos "fuser -k 3000/tcp" deb yozing.');
    }
});
