...
<head>
    ...
    <style>
        ...
        .racks-container { display: flex; gap: 15px; padding: 20px; overflow-x: auto; scroll-behavior: smooth; }
        .rack-box { background: var(--card); border: 1px solid var(--brd); border-radius: 12px; padding: 12px; min-width: 250px; }
        .floor-grid { display: flex; gap: 8px; margin-bottom: 8px; }
        
        /* KATAKLAR KVADRAT SHAKLIDA */
        .cell {
            flex: 1; 
            aspect-ratio: 1 / 1; /* KVADRAT QILISH */
        ...
        .empty { border-style: dashed; font-size: 20px; color: #444; cursor: pointer; }
        .empty:hover { border-color: #666; background: rgba(255,255,255,0.02); }

        .water-container { position: absolute; inset: 0; overflow: hidden; pointer-events: none; border-radius: 10px; }
        
        .glass-water { 
            position: absolute; bottom: -50px; left: -55%; width: 210%; 
        ...
        .qty-val { font-size: 32px; font-weight: 900; z-index: 5; pointer-events: none; }
        .code-label { position: absolute; top: 6px; font-size: 10px; font-weight: bold; opacity: 0.5; z-index: 5; text-align: center; width: 90%; overflow: hidden; }
        ...
        .btn-sm:hover { transform: scale(1.3); color: #fff; }
...
<body onclick="if(event.target==document.getElementById('modal')) closeModal()">
    ...
    <script>
        ...
        function handleSlosh(e, cell) {
            ...
            water.style.animationDuration = '6s';
        ...
        function doSearch(v) {
            ...
            let targetItem = null;
            items.forEach(it => {
                if(it.dataset.code.toLowerCase().includes(searchValue)) {
...
app.listen(3000, () => console.log('Ambar Square: http://localhost:3000'));
