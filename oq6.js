...
<head>
    ...
    <style>
        ...
        .inventory-item:hover .blick-effect {
            animation: blick 1.2s ease-out forwards;
        }
        @keyframes blick { 0% { left: -100%; } 100% { left: 150%; } }

        .shooting-star {
            position: absolute; top: -10px; left: -10px; width: 2px; height: 2px;
            background: #fff; box-shadow: 0 0 8px #fff; opacity: 0; pointer-events: none;
        }
        .inventory-item:hover .shooting-star {
            animation: star-move 0.8s ease-in-out forwards;
        }
        @keyframes star-move {
        ...
        /* STATIK HOLATDA MILTILLASH (FAQAT MUAMMO BO'LSA) */
        .qty-warning { color: var(--orange-mitter); }
        .qty-danger { color: var(--red); font-weight: 900; }
...
// Portni xavfsiz ishga tushirish
const server = app.listen(3000, () => console.log('Server running on port 3000'));
server.on('error', (e) => {
    if (e.code === 'EADDRINUSE') {
        console.log('XATO: 3000-port band. Terminalga "fuser -k 3000/tcp" deb yozing.');
    }
});
