...
<body onclick="if(event.target==document.getElementById('modal')) closeModal()">
    ...
    <div class="header">
        ...
        <input type="text" id="srch" placeholder="Artikulni yozing..." oninput="doSearch(this.value)">
    ...
    <script>
        ...
        // AUTOSCROLL VA QIDIRUV LOGIKASI
        function doSearch(v) {
            const items = document.querySelectorAll('.item');
            items.forEach(it => it.style.borderColor = ''); // Hamma hoshiyalarni tozalash
            ...
            items.forEach(it => {
                if(it.dataset.code.toLowerCase().includes(searchValue)) {
                    it.style.borderColor = '#f1c40f'; // Sariq hoshiya
                    if(!targetItem) targetItem = it; // Birinchi topilganini saqlash
                }
            });

            if(targetItem) {
                // Ekranni yumshoq siljitish (Markazga olib keladi)
...
app.listen(3000, () => console.log('Ambar Autoscroll: http://localhost:3000'));
