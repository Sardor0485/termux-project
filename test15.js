...
<head>
    ...
    <style>
        :root[data-theme="dark"] { --bg: #0b0e14; --card: #161b22; --border: #30363d; --text: #c9d1d9; --accent: #58a6ff; --header: #0d1117; }
        :root[data-theme="light"] { --bg: #f6f8fa; --card: #ffffff; --border: #d0d7de; --text: #24292f; --accent: #0969da; --header: #ffffff; }
        
        * { user-select: none; -webkit-user-select: none; box-sizing: border-box; }
        input { user-select: text; -webkit-user-select: text; }

        body { background: var(--bg); color: var(--text); font-family: sans-serif; margin: 0; padding: 15px; padding-top: 130px; transition: background 0.3s; }
        ...
        .search-input { width: 100%; padding: 10px; border-radius: 8px; border: 1px solid var(--border); background: var(--bg); color: var(--text); outline: none; }
        ...
        .add-btn-main { background: var(--accent); color: white; border: none; width: 40px; height: 40px; border-radius: 50%; font-size: 25px; cursor: pointer; }
        ...
        .more-menu { cursor: pointer; padding: 5px 10px; font-size: 20px; color: var(--text); opacity: 0.6; }
        ...
        .grid-table { display: grid; gap: 8px; align-items: center; }
        .cell-box { width: 65px; height: 65px; border: 1px solid var(--border); border-radius: 8px; background: rgba(0,0,0,0.05); }
        .etaj-label { font-size: 10px; opacity: 0.5; text-align: center; }
        
        .empty { width: 100%; height: 100%; display: flex; align-items: center; justify-content: center; cursor: pointer; }
        .empty.drag-over { background: rgba(88, 166, 255, 0.2) !important; border: 2px dashed var(--accent); }

        .inventory-item { width: 100%; height: 100%; position: relative; display: flex; flex-direction: column; align-items: center; justify-content: center; cursor: grab; transition: 0.2s; }
        .item-code { font-size: 10px; position: absolute; top: 6px; font-weight: bold; opacity: 0.7; }
        .item-qty { font-size: 22px; font-weight: bold; }
        ...
        .highlight .item-qty, .highlight .item-code { color: #000 !important; }

        .qty-ok .item-qty { color: #3fb950; }
        .qty-warning .item-qty { color: #d29922; }
        .qty-danger .item-qty { color: #f85149; }

        .controls { display: none; position: absolute; bottom: 0; width: 100%; background: rgba(0,0,0,0.8); justify-content: center; gap: 8px; padding: 5px 0; }
        .inventory-item.active .controls { display: flex; }
        .controls button { border: none; color: #fff; padding: 2px 10px; border-radius: 4px; cursor: pointer; }
        ...
        .modal-box { background: var(--card); padding: 25px; border-radius: 20px; width: 85%; max-width: 320px; text-align: center; }
...
<header>
    <div class="top-bar">
        <div style="font-weight:bold; color:var(--accent)">AMBAR PRO</div>
        <div class="nav-btns">
            <button class="theme-btn" onclick="toggleTheme(event)">🌓</button>
            <button class="add-btn-main" onclick="openConfigModal(event)">+</button>
        </div>
    </div>
    <input type="text" id="searchInput" class="search-input" placeholder="Tavar kodini qidiring..." onkeyup="searchItem(event)">
</header>

<div id="content">${htmlRows}</div>

<div id="itemModal" class="modal"><div class="modal-box" onclick="event.stopPropagation()">
    <h3>Yangi Tavar</h3><input type="text" id="newCode" placeholder="Kod"><input type="number" id="newQty" placeholder="Soni">
    <button onclick="saveItem()" style="width:100%; padding:15px; background:var(--accent); color:white; border:none; border-radius:10px; font-weight:bold;">SAQLASH</button>
</div></div>

<div id="configModal" class="modal"><div class="modal-box" onclick="event.stopPropagation()">
    <h3>Yangi Bo'lim</h3><input type="text" id="rn" placeholder="Nomi"><input type="number" id="rc" placeholder="Stellaj"><input type="number" id="sc" placeholder="Etaj"><input type="number" id="sl" placeholder="Katak">
    <button onclick="saveConfig()" style="width:100%; padding:15px; background:var(--accent); color:white; border:none; border-radius:10px; font-weight:bold;">OCHISH</button>
</div></div>
...
<script>
    ...
    document.addEventListener("dragleave", e => { if(e.target.classList.contains('empty')) e.target.classList.remove('drag-over'); }, true);
...
app.listen(3000, '0.0.0.0', () => console.log("Server ishlamoqda: port 3000"));
