...
function renderCell(itm, q, st, et, us) {
    ...
    // Bo'sh katak - unga ham narsa tashlash (Drop) mumkin
    if (!itm) {
    ...
    // To'la katak - surish (Drag) va ustiga tashlash (Drop) mumkin
    return `
...
<head>
    ...
    <style>
        ...
        .cell {
            flex: 1; height: 100px; border: 1.5 solid var(--brd); border-radius: 8px;
        ...
        /* Drag-over effekti */
        .cell.drag-over { border: 2px dashed #007bff; background: rgba(0, 123, 255, 0.1); }
        ...
        .glass-water { position: absolute; bottom: -15%; left: -15%; width: 130%; background: var(--water); z-index: 1; transition: height 0.4s; transform-origin: center bottom; }
        
        .sloshing .glass-water { animation: fast-slosh 1.2s ease-in-out forwards; }
        @keyframes fast-slosh { 0%, 100% { transform: rotate(0deg); } 25% { transform: rotate(7deg); } 50% { transform: rotate(-5deg); } }
        ...
        .m-box { background:var(--card); padding:25px; border-radius:15px; width:280px; }
...
<body>
    ...
    <div id="modal">
        <div class="m-box">
            ...
            <div style="display:flex; gap:10px">
                <button onclick="saveItem()" style="flex:1; padding:10px; background:#28a745; color:#fff; border:none; border-radius:8px; cursor:pointer; font-weight:bold">OK</button>
    ...
    <script>
        ...
        function allowDrop(e) { 
            e.preventDefault(); 
            if(e.target.classList.contains('cell')) e.target.classList.add('drag-over');
        }

        // Katakdan chiqib ketganda effektni o'chirish
        document.addEventListener('dragleave', (e) => {
            if(e.target.classList.contains('cell')) e.target.classList.remove('drag-over');
        });
...
app.post('/move', async (req,res) => {
    ...
    // Bir joyda ikki tovar bo'lmasligi uchun (oddiy almashtirish yoki ko'chirish)
...
app.listen(3000, () => console.log('Ambar Final: http://localhost:3000'));
