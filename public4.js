...
<head>
    ...
    <style>
        ...
        body { background: var(--bg); color: #adbac7; font-family: sans-serif; margin: 0; padding-top: 150px; scroll-behavior: smooth; }
        ...
        .row-title { padding-left: 20px; font-weight: 900; color: var(--blue); margin-bottom: 30px; font-size: 36px; border-left: 12px solid var(--blue); }
        .racks-container { display: flex; gap: 30px; overflow-x: auto; padding: 0 20px 30px; }
        
        .rack { background: var(--card); border: 1px solid var(--border); border-radius: 15px; padding: 20px; min-width: 440px; }
        .rack-num { text-align: center; font-size: 32px; color: #fff; margin-bottom: 20px; border-bottom: 3px solid var(--border); padding-bottom: 12px; font-weight: 800; }
        
        .shelf-wrapper { display: flex; align-items: flex-start; gap: 15px; margin-bottom: 25px; }
        .floor-num { width: 40px; font-size: 28px; font-weight: 900; color: var(--blue); text-align: center; padding-top: 35px; }
        ...
        .item-code { font-size: 15px; font-weight: 800; color: #8b949e; position: absolute; top: 10px; text-align: center; width: 100%; }
        .item-qty { font-size: 38px; font-weight: 900; margin-top: 15px; }
        
        /* PLUS-MINUS TUGMALARI PASTDA */
        .controls { 
            display: none; 
            width: 100%; 
            height: 45px; 
        ...
        .controls button { flex: 1; border: 0; color: #fff; font-size: 28px; font-weight: bold; cursor: pointer; }
        ...
        .found-cell .item-code { color: #000 !important; }
        ...
        .btn-add { width: 100%; height: 115px; border: 1px solid var(--border); border-radius: 8px; font-size: 45px; color: #30363d; display: flex; align-items: center; justify-content: center; cursor: pointer; }

        #modal { display:none; position:fixed; top:50%; left:50%; transform:translate(-50%,-50%); background:var(--card); padding:40px; border:3px solid var(--blue); border-radius:20px; z-index:2000; width: 90%; max-width: 450px; }
        #newCode { padding:20px; width:100%; background:#000; color:#fff; border:2px solid #444; margin-bottom:25px; border-radius:15px; font-size:26px; box-sizing:border-box; }
    </style>
</head>
<body>
    ...
    <div class="header">
        ...
        <input type="text" id="sInp" placeholder="Qidiruv..." oninput="handleSearch()">
    </div>

    <div id="mainContent"> ${html} </div>

    <div id="modal">
        <h3 style="margin:0 0 25px 0; color:var(--blue); font-size: 28px;">Yangi tovar</h3>
        <input type="text" id="newCode" placeholder="Kod...">
        <button style="width:100%; padding:20px; background:var(--blue); color:#fff; border:0; border-radius:15px; font-weight:bold; font-size:24px;" onclick="saveItem()">SAQLASH</button>
        <button onclick="closeModal()" style="width:100%; margin-top:20px; background:none; color:#8b949e; border:0; font-size:20px;">Bekor qilish</button>
    </div>
